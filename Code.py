...
def compress_zip(folder_path):
    archive_name = (
        f"{os.path.basename(folder_path)}_" f"{datetime.now().strftime('%Y_%m_%d')}.zip"
    )
...
def compress_tar(folder_path):
    archive_name = (
        f"{os.path.basename(folder_path)}_" f"{datetime.now().strftime('%Y_%m_%d')}.tar"
    )
...
def compress_tgz(folder_path):
    archive_name = (
        f"{os.path.basename(folder_path)}_" f"{datetime.now().strftime('%Y_%m_%d')}.tgz"
    )
...
def main():
    ...
    selected_compression_type = input(
        "Enter the number of the compression type to use: "
    )
    try:
        archive_name = compress_folder(
            folder_path, compression_types[selected_compression_type]
        )
